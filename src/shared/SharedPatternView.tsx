...
function PatternView({ pattern, owner }: { pattern: Pattern; owner: OwnerDescriptor }) {
  ...
  const handleFork = () => {
    ...
    // Snapshot the source creator's display name onto the fork so the
    // "Forked from X" line survives later changes to the source row.
    const sourceCreatorName = owner.kind === 'attributed' ? owner.displayName : null;
    forkPattern(pattern, sourceCreatorName);
      ...
      <header className="flex flex-col gap-1">
        ...
        {pattern.forkedFromId !== null && (
          <p className="text-xs font-mono text-muted-foreground/80">
            Forked from{' '}
            {pattern.forkedFromCreatorName ? (
              <Link
                to={{ kind: 'profile', displayName: pattern.forkedFromCreatorName }}
